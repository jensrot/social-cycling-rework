      ...
      for (let i = 0; i < origins.length; i++) {
        for (let j = 0; j < destinations.length; j++) {
          ...
          if (distances.rows[0].elements[j].status == 'OK') {
            ...
            return res.status(400).send({ wrongFormat: "Please choose a valid location!" })
          }
        }
      }
    }
...
exports.getAllPosts = (req, res, next) => {
  // Sort data based on the amount of likes
...
exports.getAllLatestPosts = (req, res, next) => {
  // Sort data based on the last created post
...
exports.updatePostById = (req, res, next) => {
  ...
  if (req.body.startDate) postFields.startDate = req.body.startDate;
  if (req.body.startTime) postFields.startTime = req.body.startTime;
  if (req.body.startLocation || req.body.endLocation) {
    ...
    // Recalculate
    return generateGroupRide(req.body.startLocation, req.body.endLocation, req, res);
  }
...
// TODO: Get the posts liked from the logged in user.
...
// TODO: Get the persons that liked the groupride (attendees)
exports.getLikedProfiles = (req, res) => {
  // We get the user id -> we get the profile connected to the user
...
exports.addComment = (req, res, next) => {
  // Validation happens in frontend
